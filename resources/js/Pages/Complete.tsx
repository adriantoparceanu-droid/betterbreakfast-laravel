...
const moods: { value: CheckInMood; labelKey: string; emoji: string }[] = [
    { value: 'energized', labelKey: 'complete.moodEnergized', emoji: '⚡' },
    { value: 'full',      labelKey: 'complete.moodFull',      emoji: '😊' },
    { value: 'hungry',    labelKey: 'complete.moodHungry',    emoji: '😐' },
];
                ...
                {step === 'celebrate' && (
                    <motion.div key="celebrate" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, scale: 1.04 }}
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.4 }} className="text-center">
                            <p className="text-white/70 text-base font-medium mb-1">{t('complete.day', { day: dayNumber })}</p>
                            <h1 className="text-white text-5xl font-bold tracking-tight">{t('complete.complete')}</h1>
                        </motion.div>
                        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.3 }} className="text-white/40 text-xs mt-20">{t('complete.tapToContinue')}</motion.p>
                    </motion.div>
                )}
                {step === 'checkin' && (
                    <motion.div key="checkin" initial={{ y: '100%' }} animate={{ y: 0 }} transition={{ type: 'spring', damping: 28, stiffness: 280 }}
                        ...
                        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18, duration: 0.35 }} className="w-full max-w-sm">
                            <h2 className="text-2xl font-bold text-gray-900 text-center mb-1.5">{t('complete.howDidThisFeel')}</h2>
                            <p className="text-gray-500 text-sm text-center mb-10">{t('complete.checkIn', { day: dayNumber })}</p>
                            <div className="flex flex-col gap-3">
                                {moods.map(({ value, labelKey, emoji }, i) => (
                                    <motion.button key={value} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }}
                                        ...
                                        <span className="text-lg font-semibold text-gray-800">{t(labelKey)}</span>
                                    </motion.button>
                                ))}
                            </div>
                        </motion.div>
                ...
                {step === 'done' && (
                    <motion.div key="done" initial={{ y: '100%' }} animate={{ y: 0 }} transition={{ type: 'spring', damping: 28, stiffness: 280 }}
                        ...
                        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.4 }}
                            ...
                            <h2 className="text-3xl font-bold text-gray-900 mb-3">{t('common.niceWork')}</h2>
